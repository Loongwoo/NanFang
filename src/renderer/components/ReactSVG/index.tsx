...
export default class InlineSVG extends React.PureComponent<IProps, IState> {
  ...
  private handleError = (error: InlineSVGError | IFetchError) => {
    ...
    const status =
      error.message === 'Browser does not support SVG'
        ? STATUS.UNSUPPORTED
        : STATUS.FAILED;
    ...
    try {
      ...
      return fetch(src)
        .then(response => {
          ...
          // if (
          //   !['image/svg+xml', 'text/plain'].some(d => fileType.indexOf(d) >= 0)
          // ) {
          //   throw new InlineSVGError(`Content type isn't valid: ${contentType} ${fileType}`);
          // }
        ...
        .then(content => {
          ...
          if (cacheRequests) {
            ...
            if (cache) {
              ...
              cache.queue = cache.queue.filter(
